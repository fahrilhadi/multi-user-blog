        ...
        <div class="max-w-2xl mx-auto px-4 py-10">
            ...
            <form action="{{ route('posts.update', $post->id) }}" method="POST">
                ...
                <div class="mb-4">
                    ...
                    <select name="category" id="category" class="w-full category-select">
                        <option value="">-- Select or type category --</option>
                        @foreach($categories as $category)
                            <option value="{{ $category->name }}" 
                                {{ old('category', $post->category->name ?? '') == $category->name ? 'selected' : '' }}>
                                {{ $category->name }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="mb-4">
                    ...
                    <select name="tags[]" id="tags" class="w-full tags-select" multiple>
                        @foreach($tags as $tag)
